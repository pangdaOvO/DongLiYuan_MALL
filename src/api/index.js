...
export const reqCategoryList = ()=>loc({url:'/category/getBaseCategoryList',method:'get'});
//获取banner数据的接口函数
...
// 请求超市商品数据
export const reqGetSuperList = ()=>loc({url:'/product/shoplist',method:'get'});
// 删除超市购物车数据
export const reqDeleteSuper = (shopCartId)=>loc({url:`/shopCart/delete/${shopCartId}`,method:'delete'})
// 获取超市商品详情
export const reqGetSuperDitail = (id) =>loc({url:`/product/${id}`,method:'get'});
// 获取分类商品数据
export const reqGetCategorieList = (id)=> loc({url:`/product/shoplist/${id}`,method:'get'});
// 获取购物车数据
export const reqGetSuperShopCartList = ()=>loc({url:'/shopCart/list',method:'get'})
// 超市商品加入购物车
export const reqAddShopCart = (productId,count) => loc({url:`shopCart/add/${productId}/${count}`,method:'post'})
//搜索产品的接口[真实的接口]
...
// export const reqLogin = (phone,password)=>loc({url:`/login`,method:'post',data:{phone,password}});

//保存登陆信息到本地后端
///api/user/passport/login  post phone password
export const saveToLoc = (userId)=>loc({url:`/save/${userId}`,method:'post'});
